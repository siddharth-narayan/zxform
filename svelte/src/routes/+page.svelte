<script lang="ts">
  ...
  const schema = z
    .object({
      ...
      age: z.number().gte(21, { error: "Not even old enough to drink?" }),
      ...
      isCool: z
        .boolean()
        .refine((val) => val, { error: "You can't be uncool, sorry :(" })
        .meta({ title: "Are you cool?" }),
    })
    .meta({
      title: "Example Form",
      description: "A simple form to show you what's possible",
      callback: cb,
    });
</script>
